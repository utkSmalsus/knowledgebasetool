...
(function (global, factory) {
    ...
    /*! @azure/msal-common v16.14.0 2026-09-02 */
    /*
    ...
    class CacheManager {
        ...
        removeAccessToken(key, correlationId) {
            ...
            if (credential.credentialType.toLowerCase() ===
                ...
                switch (tokenType) {
                    case AuthenticationScheme$1.POP:
                    case AuthenticationScheme$1.DPOP.toLowerCase(): {
    ...
    function buildClientConfiguration({ authOptions: userAuthOptions, systemOptions: userSystemOptions, loggerOptions: userLoggerOption, storageInterface: storageImplementation, networkInterface: networkImplementation, cryptoInterface: cryptoImplementation, tokenBindingKeyManager: tokenBindingKeyManager, clientCredentials: clientCredentials, libraryInfo: libraryInfo, telemetry: telemetry, serverTelemetryManager: serverTelemetryManager, persistencePlugin: persistencePlugin, serializableCache: serializableCache, }) {
        ...
        return {
            ...
            storageInterface: storageImplementation ||
                new DefaultStorageClass(userAuthOptions.clientId, DEFAULT_CRYPTO_IMPLEMENTATION, new Logger(loggerOptions, name$1, version$1), new StubPerformanceClient(), undefined, resolvedTokenBindingKeyManager),
    ...
    class ResponseHandler {
        ...
        async handleServerTokenResponse(serverTokenResponse, authority, reqTimestamp, request, apiId, authCodePayload, userAssertionHash, handlingRefreshTokenResponse, forceCacheRefreshTokenResponse, serverRequestId, additionalCacheKeyComponents) {
            ...
            serverTokenResponse.key_id =
                serverTokenResponse.key_id ||
                    ...
                    request.sshKid ||
                    undefined;
            if (request.authenticationScheme === AuthenticationScheme$1.DPOP) {
                if (serverTokenResponse.token_type?.toLowerCase() !==
                    AuthenticationScheme$1.DPOP.toLowerCase()) {
                    this.performanceClient?.addFields({
        ...
        static async generateAuthenticationResult(cryptoObj, authority, cacheRecord, fromTokenCache, request, performanceClient, options = {}) {
            ...
            return {
                ...
                requestId: requestId || "",
                familyId: familyId,
                tokenType: cacheRecord.accessToken?.tokenType?.toLowerCase() ===
                    AuthenticationScheme$1.DPOP.toLowerCase()
                    ? AuthenticationScheme$1.DPOP
    ...
    class RefreshTokenClient {
        ...
        async acquireTokenWithCachedRefreshToken(request, foci, apiId) {
            ...
            if (refreshTokenRequest.authenticationScheme ===
                AuthenticationScheme$1.DPOP &&
    ...
    class SilentFlowClient {
        ...
        async acquireCachedToken(request) {
            ...
            if (cachedAccessTokenType === AuthenticationScheme$1.DPOP.toLowerCase()) {
    ...
    const DEFAULT_BROWSER_CACHE_MANAGER = (clientId, logger, performanceClient, eventHandler, staticAuthorityOptions, tokenBindingKeyManager = DEFAULT_TOKEN_BINDING_KEY_MANAGER) => {
        const cacheOptions = {
    ...
    function validateDpopRequest(request, correlationId) {
        if (!request.resourceRequestMethod?.trim() ||
    ...
    async function getTokenBindingRequestParams(request, tokenBindingKeyManager, logger, performanceClient) {
        switch (request.authenticationScheme) {
            case AuthenticationScheme$1.DPOP:
                ...
                return {
                    dpopJkt: await tokenBindingKeyManager.provisionTokenBindingKey({
                        tokenBindingKeyType: AuthenticationScheme$1.DPOP.toLowerCase(),
    ...
    async function getStandardParameters(config, authority, request, logger, performanceClient) {
        ...
        if (request.platformBroker) {
            ...
            if (request.authenticationScheme ===
                AuthenticationScheme$1.POP &&
                request.reqCnf) {
                addPopToken(parameters, request.reqCnf);
            }
        }
        if (request.authenticationScheme === AuthenticationScheme$1.DPOP &&
            request.dpopJkt) {
            parameters.set(DPOP_JKT, request.dpopJkt);
        }
        instrumentBrokerParams(parameters, request.correlationId, performanceClient);
        return parameters;
    }
    ...
    class StandardController {
        ...
        constructor(operatingContext) {
            ...
            this.browserStorage = this.isBrowserEnvironment
                ? new BrowserCacheManager(this.config.auth.clientId, this.config.cache, this.browserCrypto, this.logger, this.performanceClient, this.eventHandler, buildStaticAuthorityOptions(this.config.auth), this.tokenBindingKeyManager)
                : DEFAULT_BROWSER_CACHE_MANAGER(this.config.auth.clientId, this.logger, this.performanceClient, this.eventHandler, undefined, this.tokenBindingKeyManager);
            // initialize in memory storage for native flows
            const nativeCacheOptions = {
                cacheLocation: BrowserCacheLocation.MemoryStorage,
                cacheRetentionDays: 5,
            };
            this.nativeInternalStorage = new BrowserCacheManager(this.config.auth.clientId, nativeCacheOptions, this.browserCrypto, this.logger, this.performanceClient, this.eventHandler, undefined, this.tokenBindingKeyManager);
    ...
    class NestedAppAuthController {
        constructor(operatingContext) {
            ...
            this.browserStorage = this.operatingContext.isBrowserEnvironment()
                ? new BrowserCacheManager(this.config.auth.clientId, this.config.cache, this.browserCrypto, this.logger, this.performanceClient, this.eventHandler, buildStaticAuthorityOptions(this.config.auth), tokenBindingKeyManager)
                : DEFAULT_BROWSER_CACHE_MANAGER(this.config.auth.clientId, this.logger, this.performanceClient, this.eventHandler, undefined, tokenBindingKeyManager);
